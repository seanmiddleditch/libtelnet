...
union telnet_event_t {
	...
	struct negotiate_t {
		enum telnet_event_type_t _type; /*!< alias for type */
		short telopt_extended;          /*!< option being negotiated (either EXOPL or regular) */
	...
	struct subnegotiate_t {
		...
		size_t size;                    /*!< number of bytes in buffer */
		short telopt_extended;          /*!< option code for subnegotiation (either EXOPL or regular) */
 ...
 */
extern void telnet_subnegotiation(telnet_t *telnet, short telopt,
		const char *buffer, size_t size);
